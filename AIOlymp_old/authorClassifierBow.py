...
dictionary = Dictionary.load('rucode-7.0/dictionary')
...
def process_corpus(corpus):
    ...
    tensor = FloatTensor(corpus2dense(bows, len(dictionary))).transpose(0, 1)
    return tensor  # / tensor.sum(dim=1).unsqueeze(1).repeat(1, tensor.size()[1])
...
def extract_features(data):
    ...
    return torch.cat([contexts, answers], dim=1)
...
class AuthorDataset(Dataset):
    def __init__(self):
        ...
        self.labels = FloatTensor(data['label'] == 'ai')
...
def test_loop(dataloader, model, loss_fn):
    ...
    accuracy = (tp + tn) / size
    recall = tp / (tp + fn + 1e-5) + 1e-5
    precision = tp / (tp + fp + 1e-5) + 1e-5
    f1 = 2 / (1 / recall + 1 / precision + 1e-5)
...
if __name__ == '__main__':
    ...
    optimizer = Adamax(model.parameters(), lr=5e-3)
    # scheduler = MultiStepLR(optimizer, gamma=0.1, milestones=[20, 70])
    ...
    print("Model trained!")
    torch.save(model, 'authorClassifierBow11.pth')
