def remove_punctuation(w: str) -> list:
    res = []
    current = ''
    for char in w:
        if char == '/' or char == '@':
            return []

        if char.isalpha():
            if char == 'ё':
                current += 'е'
            else:
                current += char
        elif current:  # and current != 'не':
            res.append(current)
            current = ''

        if char in '.?!':
            res.append('$')
    if current:
        res.append(current)
    return res


def count_any_of(s, es):
    count = 0
    for e in s:
        if e in es:
            count += 1
    return count


def strip_any_of(s: str, ends, default: int = 0) -> str:
    for end in ends:
        if s.endswith(end) and len(s) > len(end):
            return s[-len(end):]
    return s[-default:]


def are_similar(s1, s2) -> bool:
    if abs(len(s1) - len(s2)) > 1:
        return False

    for i in range(min(len(s1), len(s2))):
        if s1[i] != s2[i]:
            return s1[i:] == s2[i + 1:] or s1[i + 1:] == s2[i:] or s1[i + 1:] == s2[i + 1:]

    return True


def stem(w: str) -> str:
    w = strip_any_of(w, ['ом', 'ем', 'ой', 'ей', 'ами', 'ями', 'ть', 'ем', 'им', 'ишь', 'ешь', 'ите', 'ете', 'ит', 'ет',
                         'ам', 'ям', 'ум', 'юм', 'ого', 'его', 'ым', 'ом', 'ыми'], -1)
    # while count_any_of(w, 'аеиоуыэюя') > 1 and w[-1] in 'аеиоуыэюя' or w and w[-1] in 'ьъ':
    while w and w[-1] in 'аеиоуыэюяьъй':
        w = w[:-1]
    return w


with open('train_text.csv', encoding='UTF-8') as f:
    lines = f.readlines()
data = [tuple(segment.strip() for segment in line.split('\t')) for line in lines[1:]]
train_data = data[:-500]
cross_test_data = data[-500:]

pos_count = 0
neg_count = 0

word_freq = {}
for sample in train_data:
    words = []
    for segment in sample[2].split():
        for word in remove_punctuation(segment):
            word = stem(word.lower())
            if word:
                words.append(word)

    for i in range(len(words) - 1):
        word1 = words[i]
        word2 = words[i + 1]

        word_freq.setdefault((word1, word2), (0, 0))
        if sample[1] == 'Positive':
            word_freq[(word1, word2)] = word_freq[(word1, word2)][0] + 1, word_freq[(word1, word2)][1]
            pos_count += 1
        elif sample[1] == 'Negative':
            word_freq[(word1, word2)] = word_freq[(word1, word2)][0], word_freq[(word1, word2)][1] + 1
            neg_count += 1

word_freq = {word: word_freq[word] for word in word_freq if word_freq[word][0] + word_freq[word][1] > 1}

accurate = 0
for sample in cross_test_data:
    words = []
    for segment in sample[2].split():
        for word in remove_punctuation(segment):
            word = stem(word.lower())
            if word:
                words.append(word)

    pos = 1.
    neg = 1.
    for i in range(len(words) - 1):
        word1 = words[i]
        word2 = words[i + 1]
        if (word1, word2) not in word_freq:
            for w in word_freq:
                if are_similar(word1, w[0]) and are_similar(word2, w[1]):
                    word1 = w[0]
                    word2 = w[1]
                    break
            else:
                continue

        pos *= word_freq[(word1, word2)][0] / pos_count
        neg *= word_freq[(word1, word2)][1] / neg_count

    if pos >= neg:
        if sample[1] == 'Positive':
            accurate += 1
    else:
        if sample[1] == 'Negative':
            accurate += 1

    print(f'Processed item {sample[0]}')
print(accurate / len(cross_test_data))
